...
\subsection{Calculating Users' Similarity}
As shown in \ref{fitnessscore_impl}, users can be described as sets of skills they have, so the task of measuring the similarity between users can be reduced to estimating the correlation between their skill sets. A popular approach to this problem is using the \textit{Jaccard Similarity Coefficient}, a relatively simple method to determine the similitude of two sets by dividing the size of their intersection with the size of their union. The resulting value describes how similar the sets are, and thus represents how similar the respective users are. The results maximum value is one (users are identical); the minimum value is zero (users to not share any similar skills).

\subsubsection{Jaccard Similarity Coefficient}
Let $S$ be the set of all skills existing in the system. $E$ is the employee whose distance to a the reference user $R$ will be estimated. Both persons are
described as the set of skills they have.
\begin{gather*}
	S = \{Java, Ruby, C++, ...\} \\
	E = \{x \in S \ | \ \textrm{examined employee has skill x}\} \\
	R = \{x \in S \ | \ \textrm{reference employee has skill x}\} \\
\end{gather*}
The Jaccard Similarity Coefficient $j$ is defined as the size of the intersection if $E$ and $R$ divided by their union:
\begin{gather*}
...
The goal will be to find two persons to recommend to a user who is viewing \textit{Alice's} profile.\\
...
Given the task to recommend two persons that are similar to \textit{Alice}, the recommender system would choose \textit{Bob} and \textit{Charlie}.
