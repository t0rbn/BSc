...
estimations using a two-tailed heteroscedastic T-Test with a significance level of 0.05 in order to show if the persons' estimations deviate significantly from the results calculated using the algorithm. Table \ref{tab:survey_ttest} lists the algorithm's result $f_a$, the average fitness score $f_s$ estimated by the test subjects, the standard deviation in the collected data, and the values of the T-Test for each test row. As shown, there is a statistically significant difference between the algorithm's result and the subjects' estimations for the test record \textit{Charlie}. For the other four records, however, the algorithm's results do not deviate significantly from the values collected in the survey. As the scale used in the survey has a relatively low resolution (five possible values) and due to the small sample size, these results should be taken with a grain of salt.
